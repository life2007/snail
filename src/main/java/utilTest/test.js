...
function buildAndSendMsg(payloadArr){
    for (var i = 0; i < payloadArr.length; i++) {
        ...
        try{
            ...
            // 1、下发参数,透传
            var alarmConfig = payload.alarmConfig;
            //2、要控制的设备,透传
            var deviceJson = payload.devices;
            if(!alarmConfig || !deviceJson){
                error("invalid payload="+JSON.stringify(payload));
                return;
            }
            var dnDevices = deviceJson.element;
            var wifiElementDevices = deviceJson.wifiDevice;
            //3、组装消息下发
            if(dnDevices && dnDevices.length > 0){
                ...
                if (elementDeviceList && elementDeviceList.length > 0) {
                    info(5 + " --"+sceneId );
                    try {
                        integateElementDeviceData(deviceMap.elementDeviceList, alarmConfig, payload);
                    } catch (e) {
                        LogUtil.error('schedule No1 error,'+e.stack);
                    }
                    info(6 + " --"+sceneId );
                }
                //处理mqtt协议网关控灯
 ...
 * @returns
 */
function integateElementDeviceData(devices, alarmConfig, payloadJson){
    ...
    switch(type){
        ...
        case 4:{
            ...
            try {
                var downMsg = produceRoomData(devices, alarmConfig, payloadJson);
                MessageApiUtil.jsDownMsg(downMsg);
            } catch (e) {
                LogUtil.error('schedule No2 error,'+e.stack);
            }
            break;
        }
        case 5:{
 ...
 * @returns
 */
function produceRoomData(deviceArray, roomConfig, payloadJson){
    var sceneId = payloadJson.sceneId;
    info('produceRoomData 0 --'+sceneId);
    var bright = roomConfig.brightness;
    var cct = roomConfig.colorTemperature;
